...
class CartPole:
    ...
    def simulate(self, z0, K, k, Z, controller, horizon_length):
        """
        ...
        for i in range(horizon_length):
            u[:,i] = controller(z[:,i],i,K[i],k[i],Z[:,i])
...
horizon_length = 1000
x = np.arange(0,horizon_length+1,1)
X = np.sin(2*0.5*np.pi*x*1/100)
Y = np.pi*np.cos(2*0.5*np.pi*x*1/100)
Z = np.empty([4, horizon_length+1])
for i in range (len(X)):
    Z[:,i] = np.array([X[i],np.pi,Y[i],0.]) #target trajecotry
...
Q=np.eye(4)
R=0.01*np.eye(1)
P,K,p,k=solve_ricatti_equations(A, B, Q, R, Z, horizon_length)
z,u = cart.simulate(z0, K, k, Z, controller, horizon_length)
...
ax3.plot(t, X, 'r',label='desired trajectory')
ax3.plot(t, z[0,:],'b', label='actual trajectory')
legend = ax3.legend(loc='upper center', shadow=True, fontsize='x-large')
plt.title('Trajectory difference')

#%%
pre_cost = np.empty([1, horizon_length+1])
for i in range (horizon_length+1):
    pre_cost[:,i] = (z[:,i]-Z[:,i]).T@(P[i])@(z[:,i]-Z[:,i])+2*p[i].T@(z[:,i]-Z[:,i])
cost = np.empty([1, horizon_length+1])
for i in range (horizon_length):
    cost[:,i] = (z[:,i]-Z[:,i]).T.dot(z[:,i]-Z[:,i])+0.01*u[:,i].T.dot(u[:,i])
m=(z[:,1000]-Z[:,1000]).T@(z[:,1000]-Z[:,1000])
cost[:,1000]=m
real_cost=cost.T[::-1].cumsum()[::-1]

fig4, ax4 = plt.subplots(figsize=(15,10))
ax4.plot(t, pre_cost.T,'r', label='predicted cost')
ax4.plot(t, real_cost.Ta, 'b',label='actual cost')
